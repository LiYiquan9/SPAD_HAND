...
class ManoLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kargs,
        ...
        self.register_buffer(
            "th_shapedirs", torch.Tensor(np.array(smpl_data["shapedirs"].r))
        )
        self.register_buffer(
        ...
        self.register_buffer(
            "th_faces", torch.Tensor(np.array(smpl_data["f"]).astype(np.int32)).long()
        )
    ...
    def rotation_by_quaternion(self, pose_coeffs):
        ...
        full_poses = quaternion_to_axis_angle(full_quat_poses).reshape(
            batch_size, -1
        )  # [B, 16 x 3]
    ...
    def skinning_layer(self, full_rots: torch.Tensor, betas: Optional[torch.Tensor]):
        ...
        # self.v_shaped = self.th_v_template + B_S
        self.v_shaped = self.wrist_verts
        self.zero_shape_root = self.wrist_verts[0, self.root_indices, :]
        self.v_shaped[:, :778, :] = self.th_v_template + B_S
        offsets = self.v_shaped[0, self.root_indices, :] - self.zero_shape_root
        for i in range(4):
            self.v_shaped[:, 778 + i * 16 : 794 + i * 16, :] += offsets
        J = torch.matmul(self.th_J_regressor, self.v_shaped)  # (?, 16, 3)
        if betas is None:
        ...
        # TODO:
        # offsets = verts[:, self.root_indices, :] - T_P[:, self.root_indices, :]
        # offset = offsets[*torch.argmax(offsets[:, :, 0], dim=1, keepdim=True), :]
        # # offset = offsets[:, 14, :]
        # verts = torch.cat(
        #     [verts[:, :778, :], T_P[:, 778:, :] + offset],
        #     dim=1,
        # )
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def get_mano_closed_faces(self):
        ...
        return th_closed_faces  # , hand_ignore_faces_dist
