...
class RealGTDataset(Dataset):
    def __init__(self, set="test",num_cameras=8):
        ...
        labels_path = "real_data/2024-10-23_carter_250/gt/labels.json"
        
        with open(labels_path, 'r') as file:
            label_data = json.load(file)


        hists_dataset_path = "real_data/2024-10-23_carter_250/tof"
        
        if self.set == "test":
            for j in range(1,51):
                hists = json.load(open(f"{hists_dataset_path}/{j:06d}.json"))
                ...
                mano_params_pose = (matrix_to_axis_angle(torch.from_numpy(np.array(label_data[f"{j:06d}"]['mano_pose']).reshape(16, 3, 3)))[1:] - HAND_MEAN.reshape(1,15,3)).reshape(1,45).cpu().numpy()
                mano_params_shape = torch.tensor(label_data[f"{j:06d}"]['mano_shape']).reshape(1,10).cpu().numpy()
                global_trans = torch.tensor(label_data[f"{j:06d}"]['mano_translation']).reshape(1,3).cpu().numpy()
                global_rot = matrix_to_axis_angle(torch.from_numpy(np.array(label_data[f"{j:06d}"]['mano_rotation_matrix']).reshape(1, 3, 3))).reshape(1,3).cpu().numpy()
...
#     hists_target.append(torch.tensor(response))
# sensor_data = torch.stack(hists_target).cuda()

# import numpy as np

# # Load the .npz file
# path = "/home/yiquan/spad_hand/real_data/2024-10-23_carter_250/gt/000001a_hamer.npz"
# data = np.load(path)

# # Display the keys in the .npz file
# print("Keys:", data.keys())

# # Optionally, view the shape and type of each array in the .npz file
