...
import numpy as np
import wandb 
from eval_utils import eval_pose, compute_f_score
...
def eval(model, trainloader, testloader, optimizer, criterion, scheduler, epochs, rot_type="aa", save_model_interval=25):
    ...
    logging.basicConfig(filename=f'pose_estimation/results/eval/{utc_time}/evaluation_log.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filemode='w')
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(testloader):
            ...
            MPVPE, PA_MPVPE = eval_pose((vertices_pred).reshape(-1, 778, 3),(vertices_gt).reshape(-1, 778, 3))
            MPJPE, PA_MPJPE = eval_pose((joint_pred).reshape(-1, 21, 3),(joint_gt).reshape(-1, 21, 3))
            
            f_scores = compute_f_score(vertices_pred.reshape(-1, 778, 3),vertices_gt.reshape(-1, 778, 3))
            ...
            logging.info(f"formal metrics: MPVPE:{MPVPE.mean():.3f} MPJPE:{MPJPE.mean():.3f} PA_MPVPE:{PA_MPVPE.mean():.3f} PA_MPJPE:{PA_MPJPE.mean():.3f} F@5mm: {f_scores['F@5mm']:.3f}, F@15mm: {f_scores['F@15mm']:.3f}")
    ...
    # Log average metrics
    logging.info(f"Total metrics: MPVPE:{avg_mpvpe:.3f} MPJPE:{avg_mpjpe:.3f} PA_MPVPE:{avg_pa_mpvpe:.3f} PA_MPJPE:{avg_pa_mpjpe:.3f} F@5mm: {avg_f5mm:.3f}, F@15mm: {avg_f15mm:.3f}")
...
if __name__ == "__main__":
    ...
    eval(mano_estimator, trainloader, testloader, optimizer, criterion, scheduler, epochs=200, rot_type="6d", save_model_interval=50)
